...
let selectedReport = {};
const emptyReports = computed(() => reports.value.length ? false : true);
...
const updateReports = async () => {
    try {
        await store.dispatch("reports/getUserReports", store.state.auth.user.id);
...
const createIntersectionObserver = () => {
    const loadMoreMarker = document.querySelector('#loadMoreMarker');
    ...
    if (loadMoreMarker) {
        ...
        console.error('El elemento #loadMoreMarker no se encontró en el DOM');
    }
};
...
<template>
    ...
    <main class="container">
        <h1 class="center-align">Mis reportes</h1>
        ...
        <div class="empty-reports" v-show="emptyReports && !isLoadingPage">
            <span>Por aquí no hay reportes de momento 🦗</span>
        </div>

        <!-- Lista de Reportes -->
        <div class="row" v-if="!emptyReports && !isLoadingPage">
                ...
                <div class="card report-card">
                    <div class="card-content">
                        ...
                        <div class="report_details">
                            ...
                            <span class="card-reporter">Por: <span style="color: var(--BgQuaternary);">{{
                                report.user.username }}</span></span>
                        </div>
